...
const photo = createSlice({
  ...
  reducers: {
    // Tạo action creator.Mỗi key là mỗi action; state = initialState
    // Dùng redux tookit k có return
    addPhoto: (state, action) => {
      state.push(action.payload);
    },
    removePhoto: (state, actions) => {
      console.log(actions.payload);
      // const removePhotoId = actions.payload;
      // return (state = state.filter((photo) => photo.id !== removePhotoId));
      const removePhotoId = actions.payload;
      const index = state.findIndex((photo) => photo.id === removePhotoId);
      if (index !== -1) {
